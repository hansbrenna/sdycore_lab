...
def prognostic_u(u_f,u_n,u_p,v_n,us,omega_n,z_n,Ps_n,omegas_n,ub_corr,deltaKEu_surf):
        ...
        for j in prange(4,nphi+4):
            for k in prange(nP):
                ...
                if k == 0:
                    ...
                    #Keep track of change in loss of velocity from interaction with surface
                    Ecorr_adv_omegau = (((0.5*(u_n[i,j,k]+u_n[i,j,k-1])*omegas_n[i,j]-0.5*(u_n[i,j,k]+u_n[i,j,k-1])
                               *omega_n[i,j,k]))/(Ps_n[i,j]-Pf[nP-1])
                               - u_n[i,j,k]*(omegas_n[i,j]-omega_n[i,j,k])/(Ps_n[i,j]-Pf[nP-1])) #placeholder dP must be changed for inhomogeneous p-resolution
                else:
                ...
                u_f[i,j,k] = (u_p[i,j,k]+2*dt*(-(adv_uu+adv_vu+adv_omegau)+cor_u
                           -gdz_u+curv_u))
                if k == nP-1:
                    ub_corr[i,j] = (u_p[i,j,k]+2*dt*(-(adv_uu+adv_vu+Ecorr_adv_omegau)
                    +cor_u-gdz_u+curv_u))
    # Calculate difference in kinetic energy in lowest layer caused by surface interaction
    print(ub_corr.max())
    deltaKEu_surf[4:-4,4:-4] = ((Ps_f[4:-4,4:-4]-Pf[nP-1])*(0.5*(u_f[4:-4,4:-4,-1]*u_f[4:-4,4:-4,-1])-(Ps_f[4:-4,4:-4]-Pf[nP-1])*(0.5*(ub_corr[4:-4,4:-4]*ub_corr[4:-4,4:-4]))))
    return u_f,deltaKEu_surf
        ...
        for j in prange(4,nphi+4):
            for k in prange(nP):
                ...
                if k == 0:
                    ...
                    Ecorr_adv_omegaT = (T_n[i,j,k]/theta_n[i,j,k]*(((0.5*(theta_n[i,j,k]+theta_n[i,j,k-1])*omegas_n[i,j]-0.5*(theta_n[i,j,k]+theta_n[i,j,k-1])
                               *omega_n[i,j,k]))/(Ps_n[i,j]-Pf[nP-1])
                               - theta_n[i,j,k]*(omegas_n[i,j]-omega_n[i,j,k])/(Ps_n[i,j]-Pf[nP-1])))
                else:
...
if __name__ == '__main__':
    ...
    if init:
        ...
        ub_corr = np.zeros(shape=[4+nlambda+4,4+nphi+4])
        deltaKEu_surf = np.zeros(shape=[4+nlambda+4,4+nphi+4])
...
#    sys.exit()
    while t<tstop:
        ...
        u_f,deltaKEu_surf = prognostic_u(u_f,u_n,u_p,v_n,us,omega_n,z_n,Ps_n,omegas_n,ub_corr,deltaKEu_surf)
        print(1./g*(np.mean(np.average(deltaKEu_surf[4:-4,4:-4],axis=1,weights=cosphi[4:-4])))*4*pi*a*a)
