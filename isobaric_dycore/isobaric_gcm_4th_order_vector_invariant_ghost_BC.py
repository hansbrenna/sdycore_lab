...
the 2dt computational mode we apply a Robert-Asselin filter to the 
...
def prognostic_u(u_f,u_n,u_p,v_n,us,omega_n,z_n,vort,K,Ps_n,omegas_n,Fl):
    for i in range(4,nlambda+4):
        for j in range(4,nphi+4):
            for k in range(nP):
                ...
                #Non-linear coriolis term
                cor_u = ((2*OMEGA*sinphi[j])+vort[i,j,k])*v_n[i,j,k]
                #gradient of geopotential height
                gdz_u = ((g)/(a*cosphi[j])*((z_n[i-2,j,k]+8*(-z_n[i-1,j,k]+z_n[i+1,j,k])
                      -z_n[i+2,j,k])/(12*dlambda)))
                #gradient of kinetic energy term
                dk_u = ((1.0)/(a*cosphi[j])*((K[i-2,j,k]+8*(-K[i-1,j,k]+K[i+1,j,k])
                      -K[i+2,j,k])/(12*dlambda)))
                
                u_f[i,j,k] = (u_p[i,j,k]+2*dt*(-(adv_omegau)+cor_u
                           -gdz_u-dk_u-Fl[i,j,k]))
    return u_f

@jit(nopython=True,nogil=True)
def prognostic_v(v_f,v_n,v_p,u_n,vs,omega_n,z_n,vort,K,Ps_n,omegas_n,Fphi):
    for i in range(4,nlambda+4):
        for j in range(4,nphi+4):
            for k in range(nP):
                if k == 0:
                ...
                cor_v = ((2*OMEGA*sinphi[j])+vort[i,j,k])*u_n[i,j,k]
                gdz_v = (g/a*((z_n[i,j-2,k]+8*(-z_n[i,j-1,k]+z_n[i,j+1,k])
                      -z_n[i,j+2,k])/(12*dphi)))
                dk_v = (1.0/a*((K[i,j-2,k]+8*(K[i,j-1,k]+K[i,j+1,k])
                      -K[i,j+2,k])/(12*dphi)))
                
                v_f[i,j,k] = (v_p[i,j,k]+2*dt*(-(adv_omegav)-cor_v
                   -gdz_v-dk_v-Fphi[i,j,k]))             
    return v_f
...
@jit
def diag_vorticity(vort,u_n,v_n):
    for i in range(4,nlambda+4):
        for j in range(4,nphi+4):
            for k in range(nP):
                vort[i,j,k] = ((1.0/(a*cosphi[j]))*(((v_n[i-2,j,k]+8*(-v_n[i-1,j,k]+v_n[i+1,j,k])
                      -v_n[i+2,j,k])/(12*dlambda))-((cosphi[j-2]*u_n[i,j-2,k]+8*(-cosphi[j-1]
                      *u_n[i,j-1,k]+cosphi[j+1]*u_n[i,j+1,k])-cosphi[j+2]*u_n[i,j+2,k])/(12*dphi))))
    return vort

@jit
def diag_K(K,Ku,Kv,u_n,v_n,T_n):
    for i in range(4,nlambda+4):
        for j in range(4,nphi+4):
            for k in range(nP):
                Ku[i,j,k] = (1./6.)*((0.5*(u_n[i,j+1,k]+u_n[i,j-1,k]))**2+2*u_n[i,j,k])
                Kv[i,j,k] = (1./6.)*((0.5*(v_n[i+1,j,k]+v_n[i-1,j,k]))**2+2*v_n[i,j,k])
                K[i,j,k] =  (1/(a*dphi*a*cosphi[j]*dlambda))*(a*cosphi[j]*dlambda+a*dphi*0.5*(Ku[i+1,j,k]+Ku[i-1,j,k])+0.5*((a*cosphi[j+1]*dlambda+a*dphi*Kv[i,j+1,k])+(a*cosphi[j-1]*dlambda+a*dphi*Kv[i,j-1,k])))
                #K[i,j,k] = (0.5*(P[k]/(R*T_n[i,j,k]))*(u_n[i,j,k]*u_n[i,j,k]+v_n[i,j,k]*v_n[i,j,k]))
    #K[4:-4,4:-4,:] = 0.5*(u_n[4:-4,4:-4,:]*u_n[4:-4,4:-4,:]+v_n[4:-4,4:-4,:]*v_n[4:-4,4:-4,:])
    return K
...
def update_diagnostic_BCs(z_n,zf_n,omega_n,K,vort,omegas_n,us,vs,nlambda):
    ...
    K = update_periodic_BC(K)
    K = update_polar_BC(K,nlambda,vector=False)
    vort = update_periodic_BC(vort)
    vort = update_polar_BC(vort,nlambda,vector=False)
    ...
    return z_n,zf_n,omega_n,K,vort,omegas_n
...
def read_restart_file(u_n,u_p,v_n,v_p,T_n,T_p,Ps_n,Ps_p,omega_n,z_n,theta_n,omegas_n,theta_s,us,vs,day):
    ...
    u_n[4:-4,4:-4,:] = data['u_n']
    u_p[4:-4,4:-4,:] = data['u_p']
    v_n[4:-4,4:-4,:] = data['v_n']
    v_p[4:-4,4:-4,:] = data['v_p']
    T_n[4:-4,4:-4,:] = data['T_n']
    T_p[4:-4,4:-4,:] = data['T_p']
    Ps_n[4:-4,4:-4] = data['Ps_n']
    Ps_p[4:-4,4:-4] = data['Ps_p']
    omega_n[4:-4,4:-4,:] = data['omega_n']
    z_n[4:-4,4:-4,:] = data['z_n']
    theta_n[4:-4,4:-4,:] = data['theta_n']
    omegas_n[4:-4,4:-4] = data['omegas_n']
    theta_s[4:-4,4:-4] = data['theta_s']
    us[4:-4,4:-4] = data['us']
    vs[4:-4,4:-4] = data['vs']
...
#    tstop = 86400*100
    ...
    if init:
        ...
        K = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])
        Ku = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])
        ...
        z_n,zf_n,omega_n,K,vort,omegas_n = update_diagnostic_BCs(z_n.copy(),zf_n.copy(),omega_n.copy(),K,vort,omegas_n.copy(),us,vs,nlambda)
...
#    sys.exit()
    while t<tstop:
        ...
        u_f = prognostic_u(u_f,u_n,u_p,v_n,us,omega_n,z_n,vort,K,Ps_n,omegas_n,Fl)
        v_f = prognostic_v(v_f,v_n,v_p,u_n,vs,omega_n,z_n,vort,K,Ps_n,omegas_n,Fphi)
...
#        Apply Robert-Asselin filter to prognostic variables at time n
        ...
        vort = diag_vorticity(vort,u_n,v_n)
        K = diag_K(K,Ku,Kv,u_n,v_n,T_n)
        ...
        z_n,zf_n,omega_n,K,vort,omegas_n = update_diagnostic_BCs(z_n.copy(),zf_n.copy(),omega_n.copy(),K,vort,omegas_n.copy(),us,vs,nlambda)
        #K = fourth_order_shapiro_filter(K,False,1.0)
