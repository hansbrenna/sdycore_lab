...
@jit(nopython=True,nogil=True,parallel=True)
def prognostic_q(q_f,q_n,q_p,qs,u_n,v_n,omega_n):
    for i in prange(4,nlambda+4):
        for j in prange(4,nphi+4):
            for k in prange(nP):
                adv_uq = ((u_n[i,j,k]/(a*cosphi[j]))*((q_n[i-2,j,k]+8*(-q_n[i-1,j,k]+q_n[i+1,j,k])
                      -q_n[i+2,j,k])/(12*dlambda)))
                adv_vq = (((v_n[i,j,k]/(a))*((q_n[i,j-2,k]+8*(-q_n[i,j-1,k]+q_n[i,j+1,k])
                      -q_n[i,j+2,k])/(12*dphi))))
                if k == 0:
                    adv_omegaq = (((0.5*(q_n[i,j,k+1]+q_n[i,j,k])*omega_n[i,j,k+1]))/(dP[0])
                               - q_n[i,j,k]*(omega_n[i,j,k+1]-omega_n[i,j,k])/(dP[0])) #placeholder dP must be changed for inhomogeneous p-resolution
                elif k == nP-1:
                    adv_omegaq = (((qs[i,j]*omegas_n[i,j]-0.5*(q_n[i,j,k]+q_n[i,j,k-1])
                               *omega_n[i,j,k]))/(Ps_n[i,j]-Pf[nP-1])
                               - q_n[i,j,k]*(omegas_n[i,j]-omega_n[i,j,k])/(Ps_n[i,j]-Pf[nP-1])) #placeholder dP must be changed for inhomogeneous p-resolution
                else:
                    adv_omegaq = (((0.5*(q_n[i,j,k+1]+q_n[i,j,k])*omega_n[i,j,k+1]-0.5
                               *(q_n[i,j,k]+q_n[i,j,k-1])*omega_n[i,j,k]))/(dP[0])
                               - q_n[i,j,k]*(omega_n[i,j,k+1]-omega_n[i,j,k])/(dP[0]))
                q_f[i,j,k] = q_p[i,j,k]+2*dt*(-(adv_uq+adv_vq+adv_omegaq))
    return q_f
...
if __name__ == '__main__':
    ...
    if init:
        ...
        q_f = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])
        q_n = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])+1+np.random.random(size=[nlambda+8,nphi+8,nP])/1e3
        q_p = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])+1.0
        q_h = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])+1.0
#        q_mean = np.zeros(shape=[4+nlambda+4,4+nphi+4,nP])
        ...
        Vs = np.zeros(shape=[4+nlambda+4,4+nphi+4])
        qs = np.zeros(shape=[4+nlambda+4,4+nphi+4])
...
#    sys.exit()
    while t<tstop:
        ...
        q_f = prognostic_q(q_f,q_n,q_p,qs,u_n,v_n,omega_n)

    
        u_f,v_f,T_f,Ps_f = update_prognostic_BCs(u_f.copy(),v_f.copy(),T_f.copy(),Ps_f.copy(),nlambda)
        q_f = update_periodic_BC(q_f)
        q_f = update_polar_BC(q_f,nlambda,vector=False)
        ...
        q_f = fourth_order_shapiro_filter(q_f,False,tau)
        ...
        q_f = new_polar_filter(q_f,coeffs)
...
#        Apply Robert-Asselin filter to prognostic variables at time n
        ...
        q_n = Robert_Asselin_filter(q_f,q_n,q_p)
...
#            print(beta,gTE_n/gTE_p)
        ...
        q_n = update_periodic_BC(q_n)
        q_n = update_polar_BC(q_n,nlambda,vector=False)
