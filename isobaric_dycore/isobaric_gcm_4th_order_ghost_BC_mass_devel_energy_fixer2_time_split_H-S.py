...
#    sys.exit()
    while t<tstop:
        ...
        dummy0=diag_total_energy(0,TE_p,u_n,v_n,T_n)[0]
        ...
        dummy1=diag_total_energy(0,TE_p,u_f,v_f,T_f)[0]
        print('1: {}'.format(dummy0/dummy1))
        
        u_f_test = prognostic_u(u_f,u_n,u_p,v_n,0.5*(u_n[:,:,nP-1]+u_n[:,:,nP-2]),omega_n,z_n,Ps_n,omegas_n)
        v_f_test = prognostic_v(v_f,v_n,v_p,u_n,0.5*(v_n[:,:,nP-1]+v_n[:,:,nP-2]),omega_n,z_n,Ps_n,omegas_n)
        T_f_test = prognostic_T(T_f,T_n,T_p,u_n,v_n,omega_n,theta_n,Ps_n,0.5*(theta_n[:,:,nP-1]+theta_n[:,:,nP-2]),omegas_n)
        dummy2=diag_total_energy(0,TE_p,u_f_test,v_f_test,T_f_test)[0]
        print('2: {}'.format(dummy1/dummy2))
